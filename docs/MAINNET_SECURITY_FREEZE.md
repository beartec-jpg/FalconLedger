...
## 3. Ceremony gates still ops-owned

These are **not** code bugs; they block T0 until filled:

- [ ] Mainnet **network id** chosen and published
- [ ] Genesis + AIRDROP / FAUCET / DEV keys generated offline
- [ ] DEV custody multi-sig plan documented
- [ ] UNL / validator public key list finalized
- [ ] `mainnet-genesis-split.py --dry-run` green
- [ ] Private dress rehearsal completed (`MAINNET_REHEARSAL.md`) then wiped
- [ ] Portal / Neon env staged with mainnet-only airdrop guards
- [ ] Public RPC hostnames ready; faucet limits confirmed
